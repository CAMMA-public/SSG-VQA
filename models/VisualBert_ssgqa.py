...
from transformers.models.visual_bert.configuration_visual_bert import VisualBertConfig
...
class SceneToTextAttentionLayer(nn.Module):
    def __init__(self, scene_dim, text_dim, dropout_rate=0.1):
        ...
        self.scene_projection = nn.Linear(
            scene_dim, text_dim
        )  # Project scene features to text dimension

        self.cross_attention = MultiHeadAttention(
            text_dim, heads=4, dropout_rate=dropout_rate
        )
    ...
    def forward(self, text, scene):
        # Project scene embeddings to the dimensionality of text embeddings
...
class TextToSceneAttentionLayer(nn.Module):
    def __init__(self, scene_dim, text_dim, dropout_rate=0.1):
        ...
        self.cross_attention = MultiHeadAttention(
            scene_dim, heads=3, dropout_rate=dropout_rate
        )
        self.self_attention = MultiHeadAttention(
            scene_dim, heads=3, dropout_rate=dropout_rate
        )
    ...
    def forward(self, text, scene):
        ...
        attended_scene = self.layer_norm1(attended_scene + scene)  # Residual connection

        self_attended_scene = self.self_attention(
            attended_scene, attended_scene, attended_scene
        )
        self_attended_scene = self.dropout(self_attended_scene)
        self_attended_scene = self.layer_norm2(
            self_attended_scene + attended_scene
        )  # Residual connection

        return self_attended_scene
...
class VisualBertEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.scene_text_layers = nn.ModuleList(
            [
                # SceneToTextAttentionLayer(scene_dim=18, text_dim=1024, dropout_rate=0.1)
                TextToSceneAttentionLayer(scene_dim=18, text_dim=1024, dropout_rate=0.1)
                for _ in range(2)
            ]
        )
    ...
    def forward(
        ...
        # append scene knowledge into text
        scene_embs = visual_embeds[:, :, :18]

        for layer in self.scene_text_layers:
            # embeddings = layer(embeddings, scene_embs)
            scene_embs = layer(embeddings, scene_embs)

        visual_embeds = torch.cat([visual_embeds[:, :, 18:], scene_embs], -1)
...
class VisualBertLayer_cross_attn(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.chunk_size_feed_forward = config.chunk_size_feed_forward
        self.seq_len_dim = 1
        self.attention = VisualBertCrossAttention(config)
        self.intermediate = VisualBertIntermediate(config)
        self.output = VisualBertOutput(config)
