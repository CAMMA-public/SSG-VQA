...
```bash
(ssgqa)$ wget https://s3.unistra.fr/camma_public/github/ssg-qa/ssg-qa.zip
(ssgqa)$ unzip ssg-qa.zip -d ./data/qa_txt
```
#### Visual features
...
```bash
(ssgqa)$ wget https://s3.unistra.fr/camma_public/github/ssg-qa/cropped_images.zip
(ssgqa)$ wget https://s3.unistra.fr/camma_public/github/ssg-qa/roi_yolo_coord.zip
(ssgqa)$ unzip cropped_images.zip -d ./data/visual_feats
(ssgqa)$ unzip roi_yolo_coord.zip -d ./data/visual_feats
```

- Or, create the features by yourself using the images from public [CholecT45](http://camma.u-strasbg.fr/datasets): 
```bash
(ssgqa)$ python utils/feat_extract_visual.py
(ssgqa)$ python utils/feature_extract_roi.py
```
### Model weights for the SSG-QA-Net
Place the model weights in the [`checkpoints`](./checkpoints) directory
|   Model      |  Model Weights |
| :----------: | :-----:   |
| SSG-QA-Net| [download](https://s3.unistra.fr/camma_public/github/ssg-qa/ssg-qa-net.pth.tar) |
...
```bash 
(ssgqa)$ python train.py --validate=False   
```  


## Evaluation
```bash 
(ssgqa)$ python test.py --validate=True --checkpoint checkpoints/ssg-qa-net.pth.tar
```    
Evaluate the performance in different complexity: set --dataset_type=ssg-qa-roi-analysis 
```bash 
(ssgqa)$ python test.py --validate=True --dataset_type=ssg-qa-roi-analysis --checkpoint checkpoints/ssg-qa-net.pth.tar
```

## Results
The SSG-QA dataset is a multi-class classification-based VQA dataset, we report the F1-Score in the table.
| Model   | Query Object |  Query Attribute | Existence | Counting  | Zero-hop | One-hop | Single-and | Mean |
|------:|:------:|:------:|:------:|:------:|:------:|:------:|:------:| :------:|
| SSG-QA-Net | 49.4 | 60.0  | 75.8 | 28.6 | 58.2 |  52.4 | 38.4 | 54.9 |
...
## Other works that have contributed to the generation of SSG-QA dataset
```bibtex
