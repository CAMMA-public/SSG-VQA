...
def save_clf_checkpoint(
    checkpoint_dir, epoch, epochs_since_improvement, model, optimizer, Acc, final_args
):
    """
    Saves model checkpoint.
    """
...
def eval_for_f1_et_all(y_true, y_pred):
    ...
    oa = sum(cm.diagonal()) / sum(samples)

    return mAP, mAR, mAf1, wf1, oa
